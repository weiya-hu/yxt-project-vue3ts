      ...
      <template #header>
        <div class="fsc f1">
          ...
          <el-button
            type="primary"
            class="btns"
            @click="$router.push('/website/inform/dynamic/dynamicadd')"
            >添加</el-button
          >
        </div>
      </template>
          ...
          <el-table-column label="操作" width="300">
            <template #default="{ row }">
              <div class="fcs">
                <el-link
                  type="primary"
                  @click="$router.push('/website/inform/dynamic/dynamicdetails?id=' + row.id)"
                  >查看</el-link
                >
                <div class="line"></div>
...
const getUp = async () => {
  const res = await dyUp_api({ id: upId.value }).then((res) => {
    if (res.status === 1) {
      dynamicList()
    }
  })
...
const getDown = async () => {
  const res = await dyDown_api({ id: DownId.value }).then((res) => {
    if (res.status === 1) {
      dynamicList()
    }
  })
...
const getDelDate = async () => {
  dyDel_api({ id: delId.value }).then((res) => {
    if (res.status === 1) {
      dynamicList()
      delShow.value = false
    }
