<template>
  <div class="baidu_data">
    ...
    <el-card class="mycard">
      <div class="mytable">
        <el-table :data="baiduData" border>
          ...
          <el-table-column label="操作"  width="220" fixed="right">
             <template #default="{row}">
              <div class="fcc" >
                <el-link type="primary" v-if="row.status == 2" @click="$router.push('/dmp/findc/baidudata?id='+row.id)">详情</el-link>
      ...
      <MyPage :total="total" v-model:page="page" @change="baiduList" v-model:size="size"/>
...
import { baiduList_api,baiduPass_api,baiduReject_api} from '@/api/dmp/findc'
...
const baiduPass = async() => {
  baiduPass_api({id:passId.value}).then((res:res)=>{
    baiduList()
  })
}
//驳回按钮
