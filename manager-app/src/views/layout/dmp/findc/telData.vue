<template>
  <div class="phone_data">
    ...
    <el-card class="mycard">
      <div class="mytable">
        <el-table :data="phoneData" border>
          ...
          <el-table-column label="操作"  width="220" fixed="right">
             <template #default="{row}">
              <div class="fcc" >
                <el-link type="primary" v-if="row.status == 2" @click="$router.push('/dmp/findc/teldatadetail?id='+row.id)">详情</el-link>
      ...
      <MyPage :total="total" v-model:page="page" @change="phoneList" v-model:size="size"/>
...
import { phoneList_api,phonePass_api,phoneReject_api} from '@/api/dmp/findc'
...
const phonePass = async() => {
  phonePass_api({id:passId.value}).then((res:res)=>{
    phoneList()
  })
}
//驳回按钮
