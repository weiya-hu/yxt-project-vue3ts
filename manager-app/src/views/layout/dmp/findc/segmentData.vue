<template>
  <div class="segment_data">
    ...
    <el-card class="mycard">
      <div class="mytable">
        <el-table :data="segmentData" border>
...
const segmentData = ref<SData[]>([])
const segmentList = async () => {
  const res = await segmentList_api({
    current: page.value,
    size: size.value,
    ...inputSearch,
  ...
  if (res.status === 1) {
    segmentData.value = res.body.records
...
const segmentPass = async () => {
  segmentPass_api({ id: passId.value }).then(() => {
    segmentList()
  })
}
//驳回按钮
...
const refuseSuccess = async (val: string) => {
  ...
  const { status, body } = await segmentReject_api(data)
  if (status) {
...
const upUserSuccess = async (val: any) => {
  const res = await segmentUp_api({ ...val, id: upUserId.value })
  if (res.status === 1) {
    segmentList()
  }
}
