<template>
  <div class="sms_data">
    ...
    <el-card class="mycard">
            ...
            <template #default="{ row }">
              <div class="fcs">
                <el-link
                  v-if="row.status == 4"
                  type="primary"
                  @click="$router.push('/dmp/findc/msgdatadetail?id=' + row.id)"
                  >详情</el-link
                >
                <el-link
                  v-if="row.status == 1"
      ...
      <MyPage v-model:page="page" v-model:size="size" :total="total" @change="smsList" />
...
import { smsList_api, smsPass_api, smsReject_api, smsUp_api } from '@/api/dmp/findc'
...
const smsData = ref<SData[]>([])
const smsList = async () => {
  const res = await smsList_api({
    current: page.value,
    size: size.value,
    ...inputSearch,
...
const refuseSuccess = async (val: string) => {
  ...
  const { status, body } = await smsReject_api(data)
  if (status) {
