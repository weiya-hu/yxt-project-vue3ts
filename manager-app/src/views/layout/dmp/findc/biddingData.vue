<template>
  <div class="bidding_data">
    ...
    <el-card class="mycard">
      <div class="mytable">
        <el-table :data="biddingData" border>
          ...
          <el-table-column label="操作"  width="220" fixed="right">
             <template #default="{row}">
              <div class="fcc" >
                <el-link type="primary" v-if="row.status == 2" @click="$router.push('/dmp/findc/biddingdata?id='+row.id)">详情</el-link>
      ...
      <MyPage :total="total" v-model:page="page" @change="biddingList" v-model:size="size"/>
...
import { biddingList_api,biddingPass_api,biddingReject_api} from '@/api/dmp/findc'
...
const biddingData = ref<SData[]>([])
const biddingList = async ()=>{
  const res = await biddingList_api({
    current:page.value,
    size:size.value,
    ...inputSearch,
  ...
  if(res.status==1){
    biddingData.value=res.body.records
...
const biddingPass = async() => {
  biddingPass_api({id:passId.value}).then((res:res)=>{
    biddingList()
  })
}
//驳回按钮
...
const refuseSuccess=async(val:string)=>{
  ...
  const {status,body} = await biddingReject_api(data)
  if(status){
