<template>
  <div class="bidding_data">
    ...
    <el-card class="mycard">
            ...
            <template #default="{ row }">
              <div class="fcs">
                <el-link
                  v-if="row.status == 4"
                  type="primary"
                  @click="$router.push('/dmp/findc/biddingdatadetail?id=' + row.id)"
                  >详情</el-link
                >
                <el-link
                  v-if="row.status == 1"
      ...
      <MyPage v-model:page="page" v-model:size="size" :total="total" @change="biddingList" />
...
import { biddingList_api, biddingPass_api, biddingReject_api, biddingUp_api } from '@/api/dmp/findc'
