<template>
  <div class="wx_data">
    ...
    <el-card class="mycard">
      <div class="mytable">
        <el-table :data="wxData" border>
          <el-table-column type="selection" width="50" />
          <el-table-column property="user_name" label="账户名"/>
          <el-table-column property="company_name" label="客户名称" width="200px"/>
          ...
          <el-table-column label="操作"  width="220" fixed="right">
             <template #default="{row}">
              <div class="fcc" >
                <el-link type="primary" v-if="row.status == 2" @click="$router.push('/dmp/findc/wxdatadetail?id='+row.id)">详情</el-link>
      ...
      <MyPage :total="total" v-model:page="page" @change="wxList" v-model:size="size"/>
...
import { wxList_api,wxPass_api,wxReject_api} from '@/api/dmp/findc'
...
const wxData = ref<SData[]>([])
const wxList = async ()=>{
  const res = await wxList_api({
    current:page.value,
    size:size.value,
    ...inputSearch,
...
const wxPass = async() => {
  wxPass_api({id:passId.value}).then((res:res)=>{
    wxList()
  })
}
// 驳回原因
...
const refuseSuccess=async(val:string)=>{
  ...
  const {status,body} = await wxReject_api(data)
  if(status){
