...
import { overseasPurchasePage_api, overseasPass_api, overseasReject_api, overseasPurchaseUpload } from '@/api/dmp/findb'
...
const upUserSuccess = async (val: any) => {
  const { status } = await overseasPurchaseUpload({ ...val, demand_id: upUserId.value })
  status && getList()
}
</script>
