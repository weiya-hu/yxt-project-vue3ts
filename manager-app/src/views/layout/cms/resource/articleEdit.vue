      ...
      <el-form ref="aFormRef" :model="aForm" :rules="aRules" hide-required-asterisk>
        ...
        <el-form-item label="行业分类：" prop="industry_id">
          <el-radio-group v-model="aForm.industry_id">
            <el-radio :label="item.id" v-for="item in industry" :key="item.id">{{item.industry_name}}</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="下载设置：" prop="down_type" style="margin-bottom:0">
          <div class="fc">
            ...
            <el-form-item prop="qr_code" style="margin-left:110px">
              <MediaUpload
                v-if="qrShow"
                :max="1"
...
const subSuccess = async (val:AForm) => { // 文章提交成功
  loading.value = false
  const rData = {
    "content": val.text,
    "digest": val.digest,
  ...
  const { status } =  aStatus.value == 1 ? await articleSave_api({ ...rData, id }) : await articleRelease_api({ ...rData, id })
...
const aRules = {
  url:[
    { required: true, message: '请输入第三方URL链接', trigger: 'blur' },
  ],
  url_key:[
    { required: true, message: '第三方URL秘钥：', trigger: 'blur' },
  ],
  qr_code:[
    { validator: qr_codePass, trigger: 'blur'  }
  ]
}
