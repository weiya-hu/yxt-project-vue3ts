    ...
    <el-card>
      <el-form ref="myform" inline :model="inputSearch" class="inline_myform">
        ...
        <el-form-item class="btns">
          ...
          <el-button type="primary" class="search-query" @click="searchword">查询</el-button>
          <el-button class="search-query" @click="resetSearch">重置</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    <el-card class="mycard mt20">
      <div class="mytable">
        ...
        >
          ...
          >
            <template #default="{ row }">
              <el-link type="primary" @click="$router.push('/cms/resourcedet?id=' + row.id)">{{
                row.title
              }}</el-link>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作">
            <template #default="{ row }">
              ...
              <div v-if="row.status == 3" class="fcs">
                ...
                <div class="line"></div>
                <el-link type="primary" @click="$router.push('/cms/resourceedit?id=' + row.id)"
                  >编辑</el-link
                >
                <div class="line"></div>
                <el-link type="primary" @click="del(row.id)">删除</el-link>
              </div>
              <div v-if="row.status == 1" class="fcs">
                <el-link type="primary" @click="$router.push('/cms/resourceedit?id=' + row.id)"
                  >编辑</el-link
                >
                <div class="line"></div>
...
// 重置
const resetSearch = () => {
  ;(inputSearch.status = ''), (inputSearch.time = '')
  getList()
}
// 获取列表
const getList = async () => {
...
<script lang="ts">
export default { name: 'ResourceArticle' }
</script>
