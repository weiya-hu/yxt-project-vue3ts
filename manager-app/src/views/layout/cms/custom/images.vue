<template>
  <div class="mytable">
    ...
    <el-dialog v-model="dialogVisible" title="详情图片" width="510px">
      <div v-if="showImgs" class="img-dialog">
        <el-image
          v-for="(url, index) in showImgs"
          :key="url"
          style="width: 120px; height: 135px; margin-top: 10px"
          fit
          :src="url"
...
const pass = async (id: string | number) => {
  const { status } = await articlePass_api({ id })
  status && getList()
}
// 驳回弹出框
...
<script lang="ts">
export default { name: 'CustomImages' }
</script>
