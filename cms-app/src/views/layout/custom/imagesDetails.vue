<template>
  <div class="images_details">
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item :to="{ path: '/custom/images' }">图片库</el-breadcrumb-item>
...
import { imagesDel_api } from '@/api/myWork'
...
const getList = async ()=>{
  const res = await customDetails_api({id},2)
...
const sureDel = ()=>{
  //确认删除
  imagesDel_api({id:delId.value}).then((res:res)=>{
    if(res.status == 1){
      getList()
      delShow.value = false
    }
