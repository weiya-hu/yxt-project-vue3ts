<template>
  <div class="custom_poster">
        ...
        <el-table-column width="150">
          <template #default="{row}">
            <div class="fcs">
              ...
              <el-link type="primary" v-if="row.status == 4" @click="$router.push('/custom/posterDetails?id='+row.id)">查看</el-link>
    ...
    <MyDataUpUser v-model="dialogVisible" @submitSuccess="submitsuccess" :type="3"/>
  </div>
</template>
...
const sureDel = ()=>{
  //确认删除
  customDel_api({id:delId.value},3).then((res:res)=>{
    if(res.status == 1){
      getList()
      delShow.value = false
    }
...
<script lang="ts">
export default { name:'个性化内容库-海报库' }
</script>
