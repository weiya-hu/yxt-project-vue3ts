<template>
  <div class="custom_video">
        ...
        <el-table-column width="150">
          <template #default="{row}">
            <div class="fcs">
              ...
              <el-link type="primary" v-if="row.status == 4" @click="$router.push('/custom/videoDetails?id='+row.id)">查看</el-link>
    ...
    <MyDataUpUser v-model="dialogVisible" @submitSuccess="submitsuccess" :type="4"/>
  </div>
</template>
...
const sureDel = ()=>{
  //确认删除
  customDel_api({id:delId.value},4).then((res:res)=>{
    if(res.status == 1){
      getList()
      delShow.value = false
    }
